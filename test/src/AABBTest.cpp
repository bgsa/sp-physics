...
namespace NAMESPACE_PHYSICS_TEST
{
	...
	SP_TEST_METHOD(CLASS_NAME, AABB_constructor_empty_Test)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			Assert::AreEqual(Vec3(0.5f, 0.5f, 0.5f)[i], aabb.maxPoint[i], L"Wrong value.", LINE_INFO());
		}
	}

	SP_TEST_METHOD(CLASS_NAME, AABB_constructor_twoPoints_Test)
	{
		Vec3 minPoint = Vec3(0.0f, 0.0f, 0.0f);
		Vec3 maxPoint = Vec3(10.0f, 10.0f, 10.0f);
	...
	{
		Vec3 minPoint = Vec3(10.0f, 20.0f, 30.0f);
		AABB aabb = AABB(minPoint, 10.0f, 12.0f, 15.0f);
		Vec3 maxPointExpected = Vec3(20.0f, 32.0f, 45.0f);
	...
	SP_TEST_METHOD(CLASS_NAME, AABB_center_Test)
	{
		...
		Vec3 result = aabb.center();

		Vec3 expected = Vec3(5.0f, 5.0f, 5.0f);
	...
	{
		//Check on X axis
		AABB aabb1 = AABB(Vec3(0.0f, 0.0f, 0.0f), Vec3(10.0f, 10.0f, 10.0f));
		AABB aabb2 = AABB(Vec3(10.0f, 10.0f, 10.0f), Vec3(20.0f, 20.0f, 20.0f));
		CollisionStatus result = aabb1.collisionStatus(aabb2);
		Assert::IsTrue(result == CollisionStatus::INSIDE, L"Mistake!.", LINE_INFO());

		aabb1 = AABB(Vec3(0.0f, 0.0f, 0.0f), Vec3(10.0f, 10.0f, 10.0f));
		aabb2 = AABB(Vec3(9.0f, 10.0f, 10.0f), Vec3(20.0f, 20.0f, 20.0f));
		result = aabb1.collisionStatus(aabb2);
		Assert::IsTrue(result == CollisionStatus::INSIDE, L"Mistake!.", LINE_INFO());

		aabb1 = AABB(Vec3(0.0f, 0.0f, 0.0f), Vec3(10.0f, 10.0f, 10.0f));
		aabb2 = AABB(Vec3(11.0f, 10.0f, 10.0f), Vec3(20.0f, 20.0f, 20.0f));
		result = aabb1.collisionStatus(aabb2);
		Assert::IsTrue(result == CollisionStatus::OUTSIDE, L"Mistake!.", LINE_INFO());

		//Check on Y axis
		aabb1 = AABB(Vec3(0.0f, 0.0f, 0.0f), Vec3(10.0f, 10.0f, 10.0f));
		aabb2 = AABB(Vec3(0.0f, 10.0f, 10.0f), Vec3(20.0f, 20.0f, 20.0f));
		result = aabb1.collisionStatus(aabb2);
		Assert::IsTrue(result == CollisionStatus::INSIDE, L"Mistake!.", LINE_INFO());

		aabb1 = AABB(Vec3(0.0f, 0.0f, 0.0f), Vec3(10.0f, 10.0f, 10.0f));
		aabb2 = AABB(Vec3(0.0f, 11.0f, 10.0f), Vec3(20.0f, 20.0f, 20.0f));
		result = aabb1.collisionStatus(aabb2);
		Assert::IsTrue(result == CollisionStatus::OUTSIDE, L"Mistake!.", LINE_INFO());

		//Check on z axis
		aabb1 = AABB(Vec3(0.0f, 0.0f, 0.0f), Vec3(10.0f, 10.0f, 10.0f));
		aabb2 = AABB(Vec3(0.0f, 0.0f, 10.0f), Vec3(20.0f, 20.0f, 20.0f));
		result = aabb1.collisionStatus(aabb2);
		Assert::IsTrue(result == CollisionStatus::INSIDE, L"Mistake!.", LINE_INFO());

		aabb1 = AABB(Vec3(0.0f, 0.0f, 0.0f), Vec3(10.0f, 10.0f, 10.0f));
		aabb2 = AABB(Vec3(0.0f, 0.0f, 11.0f), Vec3(20.0f, 20.0f, 20.0f));
		result = aabb1.collisionStatus(aabb2);
		Assert::IsTrue(result == CollisionStatus::OUTSIDE, L"Mistake!.", LINE_INFO());
	}
	...
	SP_TEST_METHOD(CLASS_NAME, AABB_collisionStatus_Plane_Test)
	{
		...
		aabb = AABB(Vec3(0.0f, 1.0f, 0.0f), Vec3(10.0f, 10.0f, 10.0f));
	...
	{
		Sphere sphere = Sphere(Vec3(0.0f, 20.0f, 0.0f), 10.0f);
		AABB aabb = AABB(Vec3(0.0f, 0.0f, 0.0f), Vec3(10.0f, 10.0f, 10.0f));
		CollisionStatus result = aabb.collisionStatus(sphere);
		CollisionStatus expected = CollisionStatus::INLINE;
		Assert::IsTrue(result == expected, L"Mistake!.", LINE_INFO());

		sphere = Sphere(Vec3(0.0f, 18.0f, 0.0f), 10.0f);
		result = aabb.collisionStatus(sphere);
		expected = CollisionStatus::INSIDE;
		Assert::IsTrue(result == expected, L"Mistake!.", LINE_INFO());

		sphere = Sphere(Vec3(0.0f, 22.0f, 0.0f), 10.0f);
		result = aabb.collisionStatus(sphere);
		expected = CollisionStatus::OUTSIDE;
		Assert::IsTrue(result == expected, L"Mistake!.", LINE_INFO());
	}
	...
	SP_TEST_METHOD(CLASS_NAME, AABB_closestPointInAABB_Test1)
	{
		...
		Vec3 maxPoint = Vec3(10.0f, 10.0f, 10.0f);
		AABB aabb = AABB(minPoint, maxPoint);
		Vec3 point = Vec3(-1.0f, 4.0f, 5.0f);

		Vec3 result = aabb.closestPointInAABB(point);

		Vec3 expected = Vec3(0.0f, 4.0f, 5.0f);
	...
	SP_TEST_METHOD(CLASS_NAME, AABB_closestPointInAABB_Test2)
	{
		...
		Vec3 maxPoint = Vec3(10.0f, 10.0f, 10.0f);
		AABB aabb = AABB(minPoint, maxPoint);
		Vec3 point = Vec3(100.0f, 100.0f, 100.0f);

		Vec3 result = aabb.closestPointInAABB(point);

		Vec3 expected = Vec3(10.0f, 10.0f, 10.0f);
	...
	SP_TEST_METHOD(CLASS_NAME, AABB_closestPointInAABB_Test3)
	{
		...
		Vec3 maxPoint = Vec3(10.0f, 10.0f, 10.0f);
		AABB aabb = AABB(minPoint, maxPoint);
		Vec3 point = Vec3(5.0f, 7.0f, 8.0f);

		Vec3 result = aabb.closestPointInAABB(point);

		Vec3 expected = Vec3(5.0f, 7.0f, 8.0f);
	...
	SP_TEST_METHOD(CLASS_NAME, AABB_squaredDistance_Test2)
	{
		...
		Vec3 maxPoint = Vec3(10.0f, 10.0f, 10.0f);
		AABB aabb = AABB(minPoint, maxPoint);
		Vec3 point = Vec3(-2.0f, 0.0f, 0.0f);
	...
	{
		int pointsCont = 6;
		Vec3* points = ALLOC_ARRAY(Vec3, pointsCont);
		...
		Vec3List list = Vec3List(points, pointsCont);

		AABB aabb = AABB::buildFrom(list);

		Vec3 expectedMinPoint = Vec3(-1.0f, 0.0f, 0.3f);
		Vec3 expectedMaxPoint = Vec3(12.0f, 10.0f, 10.0f);
	...
	SP_TEST_METHOD(CLASS_NAME, AABB_buildFrom_sphere_Test)
	{
		...
		Vec3 expectedMinPoint = Vec3(-10.0f, -10.0f, -10.0f);
		Vec3 expectedMaxPoint = Vec3(10.0f, 10.0f, 10.0f);
	...
	{
		AABB aabb1 = AABB(Vec3(0.0f, 0.0f, 0.0f), Vec3(10.0f, 10.0f, 10.0f));
		AABB aabb2 = AABB(Vec3(20.0f, -1.0f, 2.0f), Vec3(22.0f, 5.0f, 11.0f));

		AABB result = aabb1.enclose(aabb2);

		Vec3 expectedMinPoint = Vec3(0.0f, -1.0f, 0.0f);
		Vec3 expectedMaxPoint = Vec3(22.0f, 10.0f, 11.0f);
	...
	{
		AABB aabb = AABB(Vec3(0.0f, 0.0f, 0.0f), Vec3(10.0f, 10.0f, 10.0f));
		Sphere sphere = Sphere(Vec3(-20.0f, 0.0f, 3.0f), 10.0f);

		AABB result = aabb.enclose(sphere);

		Vec3 expectedMinPoint = Vec3(-30.0f, -10.0f, -7.0f);
		Vec3 expectedMaxPoint = Vec3(10.0f, 10.0f, 13.0f);
