...
namespace NAMESPACE_PHYSICS_TEST
{
	...
	SP_TEST_METHOD(CLASS_NAME, handleCollisionResponseGPU)
	{
		...
		cl_mem physicPropertiesGPU = gpu->createBuffer(propertiesObj1, sizeof(SpRigidBody) * 2u, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE);
		...
		SpRigidBody* resultProperties = ALLOC_NEW_ARRAY(SpRigidBody, 2u);
		gpu->commandManager->readBuffer(physicPropertiesGPU, sizeof(SpRigidBody) * 2u, resultProperties);
