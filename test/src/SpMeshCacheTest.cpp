...
namespace NAMESPACE_PHYSICS_TEST
{
	...
	public:
		...
		SP_TEST_METHOD_DEF(updateGPU_2);
#endif
	};
	...
	SP_TEST_METHOD(CLASS_NAME, updateGPU_2)
	{
		GpuContext* context = GpuContextInstance;
		GpuDevice* gpu = context->defaultDevice();
		...
		for (sp_uint i = 0; i < mesh1->vertexLength() * 3; i++)
			Asserts::isCloseEnough(vertexes[i], expected[i], SP_EPSILON_THREE_DIGITS, L"Wrong value.", LINE_INFO());
